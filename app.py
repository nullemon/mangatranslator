...
async def clean_lab(file: UploadFile = File(...), gpu_cap: str = Form("100")):
    """BETA: ten versions of the no-key clean on one page, for picking.

    Nobody can tune a clean-up from a description of it — the page has to be
    looked at. Each version moves ONE lever away from the default and carries
    its number burnt into the corner, so "7 looks best" is all the feedback
    needed to fix the default. Runs entirely locally, nothing charged.

    The result shown is a contact sheet of all ten; the individual full pages
    are zipped behind /api/cleanlab-zip/{task_id} for close inspection.
    """
    if not file.content_type or not file.content_type.startswith("image/"):
        raise HTTPException(400, "Upload an image file")
    from core.gpu_throttle import set_cap as _set_gpu_cap
    ...
    ext = Path(file.filename or "img.png").suffix or ".png"
    upload_path = f"uploads/{task_id}{ext}"
    output_path = f"output/{task_id}_lab.png"
    ...
    async def run():
        try:
            ...
            def work():
                ...
                if max(h, w) > 2200:
                    s = 2200 / max(h, w)
                    src = cv2.resize(img, (int(w * s), int(h * s)),
                                     interpolation=cv2.INTER_AREA)
                for i, (num, name, desc, kw) in enumerate(CLEAN_VARIANTS):
                    ...
                    (tw2, th2), _b = cv2.getTextSize(
                        tag, cv2.FONT_HERSHEY_SIMPLEX, 1.1, 3)
                    cv2.rectangle(v, (0, 0), (tw2 + 26, th2 + 26),
                                  (255, 255, 255), -1)
                    cv2.rectangle(v, (0, 0), (tw2 + 26, th2 + 26), (0, 0, 0), 2)
                    cv2.putText(v, tag, (13, th2 + 13),
                                cv2.FONT_HERSHEY_SIMPLEX, 1.1, (0, 0, 200), 3)
                    cv2.imwrite(f"output/{task_id}_lab{num:02d}.png", v)
                    out.append((num, name, desc, v))
                _write_atomic(output_path, contact_sheet(out))
            ...
            tasks[task_id].update({
                ...
                "output_url": f"/api/result/{task_id}",
                "original_url": f"/api/original/{task_id}",
                "lab_zip_url": f"/api/cleanlab-zip/{task_id}",
            })
        except Exception as e:
...
@app.get("/api/cleanlab-zip/{task_id}")
async def clean_lab_zip(task_id: str):
    """The ten full-size versions, zipped, named by their number."""
    if task_id not in tasks or tasks[task_id].get("mode") != "cleanlab":
        raise HTTPException(404)
    from core.effects import CLEAN_VARIANTS
    buf = io.BytesIO()
    n = 0
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for num, name, _d, _kw in CLEAN_VARIANTS:
            p = f"output/{task_id}_lab{num:02d}.png"
            if os.path.exists(p):
                zf.write(p, f"{num:02d} - {name}.png")
                n += 1
    if not n:
        raise HTTPException(404, "The versions have been cleaned up — run it again")
    buf.seek(0)
    return Response(content=buf.getvalue(), media_type="application/zip",
                    headers={"Content-Disposition":
                             'attachment; filename="clean-versions.zip"'})
